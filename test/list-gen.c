...
inline int blake2b_increment_counter( eh_hash_state *S, const uint64_t inc )
{
    S->t[0] += inc;
    S->t[1] += ( S->t[0] < inc );
    return 0;
...
inline int blake2b_is_lastblock( const eh_hash_state *S )
{
    return S->f[0] != 0;
}

inline int blake2b_set_lastnode( eh_hash_state *S )
{
    S->f[1] = -1;
    return 0;
}

inline int blake2b_set_lastblock( eh_hash_state *S )
{
...
int main(void)
{
    ...
    fflush(stdout);
    FILE *fp;
    const char fileName[] = "./gpu-list-gen.cl";
    size_t source_size;
    char *source_str;
    fp = fopen(fileName, "r");
    if (!fp) {
