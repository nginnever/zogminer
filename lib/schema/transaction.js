...
const ScriptInterpreter = require('../scriptinterpreter').ScriptInterpreter
...
const VerificationError = error.VerificationError
const MissingSourceError = error.MissingSourceError
...
TransactionIn.prototype.serialize = function serialize() {
  // TODO: this might not work, check binary decleration vs example in repo
...
TransactionOut.prototype.serialize = function serialize() {
  //TODO: this should fail, same as above
  var bytes = Binary.put()
...
Transaction.prototype.calcHash = function calcHash() {
  //TODO use different sha256 module in utils
  ...
  function getTxOut(txin, n) {
    ...
    if (!fromTxOuts) {
      throw new MissingSourceError(
        // TODO write new format hash functions
  ...
  Step(
    ...
    function checkConflicts(err, count) {
      ...
      if (valueIn.cmp(valueOut) < 0) {
        // TODO: rewrite formatValue in utils
        ...
        blockChain.getConflictingTransactions(outpoints, function (err, results) {
          if (results.length) {
            if (results[0].getHash().compare(self.getHash()) == 0) {
              // TODO: rewrite formatHashAlt function
...
Transaction.prototype.getStandardizedObject = function getStandardizedObject() {
  ...
  // TODO rewrite getVarIntSize util
